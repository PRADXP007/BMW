...
export const EditorialSection: React.FC = () => {
  ...
  // SplitType character kinetic animation on tab change or scroll
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Section 07: Left Column Sticky (position: sticky; top: 0; height: 100vh) */}
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`text-left font-display uppercase tracking-tight transition-all duration-150 cursor-pointer ${
                activeTab === tab
                  ? 'text-4xl md:text-6xl text-white pl-2 border-l-4 border-m-orange'
      ...
      {/* Right Column: Naturally Scrolling Editorial Canvas (50% Pure White Background) */}
      <div className="w-full md:w-1/2 min-h-screen bg-panel-white text-carbon-black flex flex-col justify-between p-6 md:p-margin-edge pt-24 md:pt-28 z-10">
        <div>
          {/* Header with SplitType Hook */}
          <div className="mb-6 overflow-hidden">
            <h1
              ref={headlineRef}
              className="font-display text-6xl md:text-8xl tracking-tight uppercase leading-none text-carbon-black mb-2 overflow-hidden"
            >
              {current.title}
            </h1>
            <h2 className="font-mono text-xs uppercase tracking-widest text-carbon-black/60 max-w-md font-bold">
              {current.subtitle}
            </h2>
          </div>

          {/* Featured Schematic / Specimen Figure */}
          <div className="w-full my-6 border-2 border-carbon-black p-2 relative bg-white shadow-xl">
            <div className="absolute -top-3 left-3 bg-carbon-black text-white px-2 py-0.5 font-mono text-[10px] uppercase font-bold tracking-widest">
              {current.figTitle}
            </div>
            <img
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_2.5fr] gap-6 mt-6">
            <div>
              <span className="font-mono text-[11px] text-white bg-m-orange px-2.5 py-1 inline-block uppercase tracking-widest font-bold">
                {current.tag}
              </span>
            </div>
            <div className="font-mono text-xs md:text-sm text-carbon-black/80 leading-relaxed space-y-4 font-normal">
              <p>{current.p1}</p>
              <p>{current.p2}</p>
            </div>
          </div>
        ...
        {/* Footer Meta */}
        <div className="mt-12 pt-4 border-t border-carbon-black/15 flex justify-between items-center text-[10px] text-carbon-black/50 uppercase font-mono">
