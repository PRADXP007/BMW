...
export const Preloader: React.FC = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-50 bg-[#0D0D0D] text-on-surface flex flex-col justify-between p-6 md:p-margin-edge overflow-hidden h-screen w-screen selection:bg-m-orange selection:text-white"
    >
      {/* Dark wet-asphalt motion background */}
      <div className="absolute inset-0 w-full h-full z-0 pointer-events-none">
        <img
          alt="BMW M Concept wet asphalt dark background"
      ...
      <div className="relative z-10 w-full flex justify-between items-start pt-2">
        ...
        <div className="font-mono text-xs uppercase text-on-surface-variant tracking-wider flex items-center gap-4">
          <span>
            BUFFER: <strong className="text-white">{loadProgress}%</strong>
          </span>
          <span className="hidden sm:inline">
            SYSTEM_READY: <span className="text-emerald-400 font-bold">{isCacheReady ? 'TRUE' : 'FALSE'}</span>
          </span>
        </div>
      </div>

      {/* 4 Staggered Lines: clamp(3.5rem, 8vw, 10rem); letter-spacing: -0.04em; line-height: 0.88 */}
      <div
        ref={textStackRef}
      ...
      {/* Bottom Row: Monospace Caption & #E4492E CTA Button */}
